...
void preambStatic(short choice, std::ifstream& file, int& size);
void preambDynamic(short choice, std::ifstream& file, int& size);
			...
			do
			{
				...
				if (choice != 4)
				{
					...
					default:
					{
						int size;
						std::ifstream file("data.txt");
						preambDynamic(choice, file, size);
...
void preambStatic(short choice, std::ifstream& file, int& size)
{
	...
	case 1:
	{
		std::cout << " \nМатрица из "<<Rows1<<" строк и "<<Colums1<<" столбцов ";
		std::cin.ignore(std::cin.rdbuf()->in_avail());
		break;
	}
	case 2:
